import { windowNames, smiteClassId } from "../consts";
import {
  OWGames,
  OWGameListener,
  OWWindow
...
class BackgroundController {
  ...
  public async run() {
    ...
    const currWindow = await this.isSmiteRunning() ? windowNames.inGame : windowNames.desktop;
    this._windows[currWindow].restore();
  }

  private toggleWindows(info) {
    if (!info || !this.isGameSmite(info)) {
      return;
    }
  ...
  private async isSmiteRunning(): Promise<boolean> {
    const info = await OWGames.getRunningGameInfo();

    return info && info.isRunning && this.isGameSmite(info);
  }

  // Identify whether the RunningGameInfo object we have references Fortnite
  private isGameSmite(info: RunningGameInfo) {
    return info.classId === smiteClassId;
  }
}
